...
const FilialBookmark = ({ filialId, updatedBookmarks, filialText}) => {
    ...
    const sendArchiveRequest = async () => {
        setLoading(true);
        try {
            const response = await fetch(`https://bizdincargo.kz:3001/api/archive/${userId}/archive`, {
                method: 'POST',
                headers: {
    ...
    const removeBookmark = async (trackNumber) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://bizdincargo.kz:3001/api/bookmark/${userId}/delete/${trackNumber}`, {
                    method: 'DELETE',
                });
